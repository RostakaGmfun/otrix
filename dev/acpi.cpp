...
static kerror_t acpi_validate_checksum(const struct acpi_sdt_hdr *sdt)
{
    uint8_t checksum = 0;
    ...
    return (checksum == 0 ? E_OK : E_INVAL);
}
...
static kerror_t acpi_parse_rsdt(const acpi_rsdt *rsdt)
{
    if (acpi_validate_checksum(&rsdt->hdr) != E_OK) {
        immediate_console::print("Failed %d\n", __LINE__);
        return E_INVAL;
    }
    ...
    for (int i = 0; i < num_entries; i++) {
        kerror_t ret = acpi_parse_sdt_entry(rsdt->sdt_pointers[i]);
        if (ret != E_OK) {
            immediate_console::print("Failed %d\n", __LINE__);
            return ret;
        }
